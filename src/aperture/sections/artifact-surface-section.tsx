...
function ArtifactContentPanel() {
	return (
		<div className="relative isolate overflow-hidden border border-border-strong bg-surface px-5 py-7 sm:px-8 sm:py-9">
			...
			/>
			<div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(18rem,0.7fr)] lg:items-start">
				<div className="min-w-0">
					...
					<div className="mt-6 grid gap-3 sm:grid-cols-3">
						{surfaceDetails.map((detail) => (
							<div
								className="min-w-0 border border-border bg-background/55 p-4"
								key={detail.label}
							>
								<p className="m-0 font-oz-mono text-[0.65rem] text-dim uppercase tracking-[0.14em]">
									{detail.label}
								</p>
								<p className="mt-2 mb-0 font-oz-display text-xl text-accent uppercase">
									{detail.value}
								</p>
							</div>
						))}
				...
				<form className="min-w-0 border border-border-strong bg-background/70 p-4 sm:p-5">
